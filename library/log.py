...
def log(level: LogLevel, msg: str):
    ...
    print("{}{}{}".format(f"[{ANSI_COLORS[level]}{level.name}\x1b[0m]", " " * (8 - len(level.name)), msg), file=sys.stderr)
def debug(msg: str): log(LogLevel.DEBUG, msg)
def info(msg: str): log(LogLevel.INFO, msg)
def success(msg: str): log(LogLevel.SUCCESS, msg)
def warn(msg: str): log(LogLevel.WARN, msg)
def error(msg: str): log(LogLevel.ERROR, msg)
